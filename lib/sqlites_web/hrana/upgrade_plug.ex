defmodule SqlitesWeb.Hrana.UpgradePlug do
  @moduledoc """
  ...
  `wss://host` — and hands them to `SqlitesWeb.Hrana.Socket`. All other
  ...
  def call(conn, _opts) do
    if websocket_upgrade?(conn) do
      case negotiate_subprotocol(conn) do
        {:ok, conn} ->
          conn
          |> WebSockAdapter.upgrade(SqlitesWeb.Hrana.Socket, [], timeout: :timer.minutes(10))
          |> halt()
