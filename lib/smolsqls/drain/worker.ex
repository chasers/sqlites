defmodule Smolsqls.Drain.Worker do
  @moduledoc """
  ...
  defp execute(%Request{node: node, kind: "evacuate"} = request) do
    if node_alive?(node) do
      ...
      Smolsqls.Telemetry.node_operation("evacuate", :cancelled)
      complete(request, error: "cancelled: node reconnected")
    else
      case Smolsqls.Failover.evacuate(node) do
        {:ok, %{reassigned: reassigned}} ->
          Smolsqls.Telemetry.node_operation("evacuate", :ok, reassigned)
          complete(request, reassigned: reassigned)

        {:error, reason} ->
          Logger.error("evacuation of #{node} failed: #{inspect(reason)}")
          Smolsqls.Telemetry.node_operation("evacuate", :error)
  ...
  defp execute(%Request{node: node} = request) do
    case Drain.drain(node) do
      {:ok, %{reassigned: reassigned}} ->
        Smolsqls.Telemetry.node_operation("drain", :ok, reassigned)
        complete(request, reassigned: reassigned)

      {:error, reason} ->
        Logger.error("drain of #{node} failed: #{inspect(reason)}")
        Smolsqls.Telemetry.node_operation("drain", :error)
        complete(request, error: String.slice(inspect(reason), 0, 250))
    end
  ...
  defp schedule_poll do
    interval = Application.get_env(:smolsqls, __MODULE__, [])[:poll_interval] || :timer.seconds(5)
