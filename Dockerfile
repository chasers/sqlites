...
COPY --from=builder --chown=nobody:root /app/_build/${MIX_ENV}/rel/smolsqls ./

COPY --from=docker.io/litestream/litestream:0.5 /usr/local/bin/litestream /usr/local/bin/litestream
