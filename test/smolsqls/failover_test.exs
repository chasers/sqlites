defmodule Smolsqls.FailoverTest do
  ...
  test "evacuate/1 reassigns a dead node's databases to survivors" do
      ...
      for _ <- 1..3 do
        ...
        {:ok, database} =
          ...
          |> Database.placement_changeset(%{
            status: :active,
            node: "smolsqls@dead-node",
            file_path: "/var/lib/smolsqls/data/#{tenant.id}/#{database.id}.db"
          })
          |> Repo.update()
    ...
    assert {:ok, %{reassigned: 3}} = Failover.evacuate("smolsqls@dead-node")
  ...
  test "activation restores the litestream replica when the file is missing" do
    ...
    previous = Application.get_env(:smolsqls, Smolsqls.DataPlane.Litestream)

    Application.put_env(:smolsqls, Smolsqls.DataPlane.Litestream,
      enabled: true,
    ...
    on_exit(fn ->
      case previous do
        nil -> Application.delete_env(:smolsqls, Smolsqls.DataPlane.Litestream)
        value -> Application.put_env(:smolsqls, Smolsqls.DataPlane.Litestream, value)
      end
  ...
  test "activation for a non-replicated database falls back to its latest backup" do
    ...
    {:ok, _backup} = Smolsqls.Backups.trigger(database)
