defmodule Smolsqls.DataPlane.FenceTest do
  ...
  test "stops a misplaced server only after two consecutive sightings" do
    ...
    {:ok, _} =
      database
      |> ControlPlane.Database.placement_changeset(%{status: :active, node: "smolsqls@elsewhere"})
      |> Repo.update()
  ...
  test "ignores servers without a placement row" do
    ...
    on_exit(fn -> Smolsqls.DataPlane.delete_local_files(file_path) end)

    {:ok, pid} = Smolsqls.DataPlane.Supervisor.start_database(database_id, file_path)
    on_exit(fn -> Smolsqls.DataPlane.Supervisor.stop_database(database_id) end)
